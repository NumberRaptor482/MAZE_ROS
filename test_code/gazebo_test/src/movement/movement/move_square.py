...
class MoveSquare(Node):
    ...
    def rotate(self):
        theta_diff = abs(self.position['theta']) - abs(self.target_angle)
        print(theta_diff, self.position['theta'], self.target_angle)
        return (abs(theta_diff) <= 0.5, (0.0, (-1 if theta_diff < 0 else 1) * 0.5))
